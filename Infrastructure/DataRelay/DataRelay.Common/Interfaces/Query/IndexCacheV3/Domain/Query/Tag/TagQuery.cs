...
{
    public class TagQuery : IPrimaryRelayMessageQuery,
        ISplitable<TagQueryResult>
    {
        #region Data Members
        ...
        public byte QueryId
        {
            get
            {
                return (byte)QueryTypes.TagQuery;
            }
        }
        ...
        public List<IPrimaryRelayMessageQuery> SplitQuery(int numClustersInGroup)
        {
            ...
            {
                TagQuery tagQuery;
                List<IPrimaryRelayMessageQuery> tagQueryList = new List<IPrimaryRelayMessageQuery>(numClustersInGroup);
        ...
        public TagQueryResult MergeResults(IList<TagQueryResult> partialResults)
        {
            TagQueryResult finalResult = default(TagQueryResult);
            ...
            else
            {
                ...
                for (int i = 0; i < partialResults.Count; i++)
                {
                    ...
                    else if (partialResults[i].TypeExists)
                    {
                        ...
                        for (int j = 0; j < partialResults[i].TagNames.Length; j++)
                        {
                            if (!tagNames.Contains(partialResults[i].TagNames[j]))
                            {
                                tagNames.Add(partialResults[i].TagNames[j]);
                            }
                        }
                    }
                }
                ...
                if (tagNames.Count > 0)
                {
                    finalTagNames = new string[tagNames.Count];
                    tagNames.CopyTo(finalTagNames, 0);
                }

                finalResult = new TagQueryResult
                                  {
