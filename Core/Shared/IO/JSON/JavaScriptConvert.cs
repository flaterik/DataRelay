...
using System.Xml;
using MySpace.Common.IO.JSON.Converters;
...
namespace MySpace.Common.IO.JSON
{
	...
	public static class JavaScriptConvert
	{
		...
		{
            // check below was removed as a DateTime of 1/1/0001 was coming back
            // as 1/1/0100 and was failing checks, the JavaScript Date object can
            // only hold dates >= 1/1/0100, so if anyone wants to instantiate such
            // a thing this needs to be reinstated.
		...
		public static object DeserializeObject(string value, Type type, params JsonConverter[] converters)
		{
			...
			if (!CollectionUtils.IsNullOrEmpty<JsonConverter>(converters))
			{
				for (int i = 0; i < converters.Length; i++)
				{
                    jsonDeserializer.Converters.Add(converters[i]);
				}
			}
			...
			{
				deserializedValue = jsonDeserializer.Deserialize(jsonReader, type);
			}
